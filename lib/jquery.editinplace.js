...
$.fn.editInPlace.defaults = {
	...
	load_button:		'<button class="inplace_load">Ask Google</button>', // string: image button tag to use as “Load” button
...
$.extend(InlineEditor.prototype, {
	...
	init: function() {
		this.dom.parent().removeClass( 'opened' );
	...
	handleClickOnClosedEditor: function() {
		...
		// TODO: Allow HTML editing.
//		this.originalText = trim(this.dom.text());
		this.originalText = trim( this.originalHTML );
	...
	replaceContentWithEditor: function() {
		this.dom.parent().addClass( 'opened' );
		var buttons_html  = '<div class="buttons">' + ( this.settings.show_buttons ? [ this.settings.save_button, this.settings.cancel_button, this.settings.load_button ].join(' ') : '' ) + '</div>';
	...
	connectEventsToEditor: function() {
		...
		function loadEditorAction() {
			// TODO: Load Editor contents.
			return false; // stop event bubbling
		}
		...
		form.find(".inplace_load").click(loadEditorAction);
	...
	handleSubmitToCallback: function(enteredText) {
		...
		var newHTML = this.settings.callback.call( this.dom, enteredText, this.originalHTML, this.settings.params);
		this.init();
